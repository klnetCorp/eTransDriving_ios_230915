...
@property (nonatomic, assign) BOOL captureCropMode; //저해상도, 라이브모드 off 로 동작함.
...
@property (nonatomic, weak) IBOutlet UISlider *verticalSlider;
@property (nonatomic, weak) IBOutlet UISlider *horizontalSlider;
...
@property (nonatomic) CGRect metaRect;
...
- (IBAction)capturePhoto:(id)sender
{
    ...
    self.cropRect = self.guideView.frame;
    self.metaRect = [self.previewView.videoPreviewLayer metadataOutputRectOfInterestForRect:self.cropRect];
    ...
    dispatch_async( self.sessionQueue, ^{
        ...
        if (currentPosition == AVCaptureDevicePositionFront || !currentVideoDevice.hasFlash) {
        ...
        AVCamPhotoCaptureDelegate *photoCaptureDelegate = [[AVCamPhotoCaptureDelegate alloc] initWithRequestedPhotoSettings:photoSettings willCapturePhotoAnimation:^{
            ...
            dispatch_async( self.sessionQueue, ^{
                ...
                NSData *data = [photoCaptureDelegate getPhotoData];
                
                NSData *cropData = [self cropCapturedImage:data];
                
                if(self.delegate!=nil && [self.delegate respondsToSelector:@selector(onStillImageSaved:data:)]) {
                    [self.delegate onStillImageSaved:self data:cropData];
                }
            } );
        }];
...
- (UIImage *) imageFromSampleBuffer:(CMSampleBufferRef) sampleBuffer {
    ...
    UIImage *originImage =  [UIImage imageWithCGImage:quartzImage scale:1.0 orientation:UIImageOrientationRight];
...
- (NSData *)cropCapturedImage:(NSData *)orgData {
    
    UIImage *orgImage = [UIImage imageWithData:orgData];
    CGSize originalSize = CGSizeZero;
    
//    CGRect visibleLayerFrame = self.cropRect;
//    self.metaRect = [self.previewView.videoPreviewLayer metadataOutputRectOfInterestForRect:visibleLayerFrame];
    
    if(orgImage.imageOrientation==UIImageOrientationLeft || orgImage.imageOrientation==UIImageOrientationRight) {
        
        originalSize = CGSizeMake(orgImage.size.height, orgImage.size.width);
    } else {
        originalSize = orgImage.size;
    }
    
    CGRect cropRect = CGRectIntegral(CGRectMake(self.metaRect.origin.x * originalSize.width, self.metaRect.origin.y * originalSize.height
                                                , self.metaRect.size.width * originalSize.width, self.metaRect.size.height*originalSize.height));
    CGImageRef finalCgImage = CGImageCreateWithImageInRect([orgImage CGImage], cropRect) ;
    
    UIImage *cropImage = [UIImage imageWithCGImage:finalCgImage scale:1.0f orientation:orgImage.imageOrientation];
    CGImageRelease(finalCgImage);
    
    return UIImageJPEGRepresentation(cropImage, 100);
}

- (UIImageView*)setupGuideLineArea {
    
    UIImage* resizableImage = [[UIImage imageNamed: @"guideImage"] resizableImageWithCapInsets: UIEdgeInsetsMake(10, 10, 10, 10) resizingMode: UIImageResizingModeStretch];

    UIImageView *imageView = [[UIImageView alloc] initWithImage:resizableImage];
    CGRect ivFrame = CGRectMake(0, 0, 100, 50);
    
    CGRect bounds = self.previewView.bounds;
    ivFrame.origin.x = (bounds.size.width - ivFrame.size.width) / 2;
    ivFrame.origin.y = (bounds.size.height - ivFrame.size.height) / 2;
...
//    UIImageView *guideView = [self setupGuideLineArea];
//    [self.previewView addSubview:guideView];

    self.guideView.layer.borderColor = [[UIColor greenColor] CGColor];
    self.guideView.layer.borderWidth = 2.0f;
    
    CGRect hFrame = self.horizontalSlider.frame;
    CGRect vFrame = hFrame;

    vFrame.origin.x = ([[UIScreen mainScreen] bounds].size.width)/2;
    vFrame.origin.y = hFrame.origin.y - hFrame.size.height - hFrame.size.width;
    
    self.verticalSlider.frame = vFrame;

    CGAffineTransform trans = CGAffineTransformMakeRotation(M_PI*0.5);
    [self.verticalSlider setTransform:trans];
    

    [self.verticalSlider addTarget:self action:@selector(verticalValueChanged:) forControlEvents:UIControlEventValueChanged];
    [self.horizontalSlider addTarget:self action:@selector(horizontalValueChanged:) forControlEvents:UIControlEventValueChanged];
    
    CGFloat v = 0.2f;
    CGFloat h = 0.8f;
    [self.verticalSlider setValue:v];
    [self resizeVertical:v];
    [self.horizontalSlider setValue:h];
    [self resizeHorizontal:h];

}

- (IBAction)verticalValueChanged:(UISlider *)sender {
    NSLog(@"vertical value = %f", sender.value);
    [self resizeVertical:sender.value];
}

- (IBAction)horizontalValueChanged:(UISlider *)sender {
    NSLog(@"horizontal value = %f", sender.value);
    [self resizeHorizontal:sender.value];
}

- (void)resizeVertical:(CGFloat)percent {
    CGRect frame = self.guideView.frame;
    CGFloat half = [[UIScreen mainScreen] bounds].size.width / 2;
    CGFloat start = 50;
    frame.size.width = start + half * percent;
    frame.origin.y = (start - frame.size.width) / 2;
    
    self.heightConstraint.constant = frame.size.width;
    self.cropRect = self.guideView.frame;
}

- (void)resizeHorizontal:(CGFloat)percent {
    CGRect frame = self.guideView.frame;
    CGFloat half = [[UIScreen mainScreen] bounds].size.width / 2;
    CGFloat start = 100;
    frame.size.width = start + half * percent;
    frame.origin.x = (half - frame.size.width) / 2;
    
    self.widthConstraint.constant = frame.size.width;
